...
const handlerHeaderV2Template = `// Code generated by gooze-starter. DO NOT EDIT.
...
const handlerContentV2Template = `
...
{{end}}{{if .RequestType}}	var req {{.DtoPackageName}}.{{.RequestType}}
	...
	return nil{{end}}{{else}}if handled, err := {{.LcName}}CustomHandler.{{ .RouteName }}Hook(ctx); handled {
		return err
	} else if err != nil {
		return err
    }
...
func (self *generator) GenHandlerV2() (err error) {
	...
	for _, service := range self.services {
		...
		if err = self.combineHandlerWriteV2(service, filename); err != nil {
			return err
		}
	}
...
func (self *generator) combineHandlerWriteV2(service *serviceSpec, filename string) error {
	hasDto := false
	hasRestFul := false
	hooks := []HookConfig{}
	...
	if _, err := os.Stat(filename); errors.Is(err, os.ErrNotExist) {
		headerStr, err := executeTemplate(handlerHeaderV2Template, headerData)
		if err != nil {
			return err
		}
	...
	if !strings.Contains(string(content), "type "+gzutil.UcFirst(service.Name)+"CustomHandler interface") {
		hookStr1, err := executeTemplate(`
	// 每个 handler 对应一个 Hook 变量，通过该 hook 变量可定制实现自定义内容 
	type {{.UcName}}CustomHandler interface {
	{{- range .Hooks}}
		{{.RouteName}}Hook(ctx *gin.Context{{if .RequestType}}, req *{{.DtoPackageName}}.{{.RequestType}}{{end}}) (bool, error)
	...
	for _, route := range service.Routes {
		...
		contentStr, err := executeHandlerTemplate(handlerContentV2Template, handlerData)
		if err != nil {
			return err
		}
...
{{- if .HookName }}func (self *{{ .UcName }}CustomHandlerImp) {{ .HookName }}(ctx *gin.Context{{if .RequestType}}, req *{{.DtoPackageName}}.{{.RequestType}}{{end}}) (bool, error) {
...
func (self *generator) combineCustomWrite(service *serviceSpec, filename string) error {
	...
	if _, err := os.Stat(customFile); errors.Is(err, os.ErrNotExist) {
		...
		self.hookList = append(self.hookList, gzutil.UcFirst(service.Name))
	}

	content, _ := os.ReadFile(customFile)
	if !strings.Contains(string(content), "func New"+gzutil.UcFirst(service.Name)+"CustomHandlerImp()") {
		hookStr1, err := executeTemplate(`// {{ .UcName }}CustomHandlerImp 由开发者实现，支持依赖注入。
type {{ .UcName }}CustomHandlerImp struct {
	// 可以在这里声明依赖，并且在 New{{ .UcName }}CustomHandlerImp() 中进行构造
}
...
`, map[string]interface{}{"UcName": gzutil.UcFirst(service.Name)})
		if err != nil {
			return err
		}
	...
	for _, route := range service.Routes {
		...
		hookStr, err := executeTemplate(`func (self *{{ .UcName }}CustomHandlerImp) {{ .HookName }}(ctx *gin.Context{{if .RequestType}}, req *{{.DtoPackageName}}.{{.RequestType}}{{end}}) (bool, error) {
	// TODO: add your custom logic, e.g. file upload, validation, DI service usage
	return false, nil
}
`, hookData)
